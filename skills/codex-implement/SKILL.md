...
State persisted under `.claude/skills/codex-implement/state/<sanitized-target>.{thread,review.txt,events.ndjson}` (the `.review.txt` file holds Codex's implementation **report** — the naming comes from the shared helpers). This skill's `scripts/start.sh` and `scripts/resume.sh` are the adapter entry points; for `reset`/`show` (shared `codex-plan-review` scripts, no adapter wrapper) export first:
...
- **Fixes are the requester's job.** After Codex reports, the requester (`addw-implement` self-review) fixes problems directly in the tree — do NOT ping-pong fixes back to Codex. Resume only for genuinely new scope (next phase, large remainder).
