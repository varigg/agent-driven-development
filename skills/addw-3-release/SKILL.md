...
All must be green. For the changelog entry's review line, read the verdict and round count from the code-review adapter's state (`.claude/skills/${ADDW_CODE_REVIEW_SKILL:-codex-code-review}/state/`, key derived from the plan path); if absent, the review was skipped — record that explicitly.
...
Plan: `docs/1-plans/F_x.y.z_feature-name.plan.md` (or "unplanned") · Review: Codex loop, N rounds → verdict [plus any overrides or accepted open findings; or "skipped — trivial change"]
```
...
4. **Reconciliation sweep** — mandatory whenever the release changed any documented mechanism. The plan's **Doc Impact** list is the starting point, not the whole job:
   ...
   - Dated records (everything under `docs/adr/`, CHANGELOG.md entries, per-release tutorials, promoted plans) are exempt: their date is part of their meaning. Do not retro-edit them. If this release invalidated a decision an ADR records, write a **superseding ADR** — never edit the old one.
...
```bash
git status
git add "$ADDW_VERSION_FILE" README.md CHANGELOG.md docs/1-plans/<plan-file> docs/ARCHITECTURE.md docs/adr/
