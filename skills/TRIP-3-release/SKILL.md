...
5. **Vocabulary sweep**: for each ADR flipped or superseded in step 3, grep **every** living doc — the whole of `docs/` outside the numbered per-release directories, plus CLAUDE.md and README.md — for the vocabulary it retires: names of removed components, superseded approaches. Enumerate the directory rather than working from a remembered list; living documents get added, and the one nobody lists is the one that keeps the retired wording. Every hit must be a dated record, an explicit negation, or a standing lesson; anything else is doc drift — fix it in this release.
